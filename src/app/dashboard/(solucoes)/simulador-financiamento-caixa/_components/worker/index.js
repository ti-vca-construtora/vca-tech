...
async function processSimulacao(job) {
  ...
  try {
    ...
    await page.waitForSelector('#origemRecurso', { state: 'visible', timeout: 10000 })
    ...
    await page.waitForSelector('#categoriaImovel', { state: 'visible', timeout: 10000 })
    ...
    valorAvaliacaoNumeros = Math.floor(Number(valorAvaliacaoNumeros) / 100).toString()
    ...
    console.log(`👥 Selecionando ${dados.quantidadeParticipantes} participantes`)
    ...
    console.log(`🔍 Selecionando código do sistema baseado em origem: ${dados.origemRecurso}`)
    ...
    const spanSelecionado = await page.evaluate((codigo) => {
      ...
      return { sucesso: false, erro: `Span com código ${codigo} não encontrado` }
    }, codigoSistema)
    ...
    console.log(`💰 Selecionando sistema de amortização: ${dados.sistemaAmortizacao}`)
    
    await page.waitForSelector('#rcrRge', { state: 'visible', timeout: 5000 })
    
    const tipoAmortizacao = dados.sistemaAmortizacao.toUpperCase().includes('SAC') ? 'SAC' : 'PRICE'
    ...
    if (dados.origemRecurso === 'FGTS') {
      ...
      console.log(`👨‍👩‍👧‍👦 Possui dependentes: ${dados.possuiDependentes ? 'Sim' : 'Não'}`)
      
      if (dados.possuiDependentes) {
        await page.waitForSelector('#possuiMaisUmParticipante', { state: 'visible', timeout: 5000 })
      ...
      await page.waitForSelector('#areaUtil', { state: 'visible', timeout: 5000 })
      await page.fill('#areaUtil', '0')
      console.log('✅ Área útil preenchida')
      await page.waitForTimeout(500)
    } else {
      console.log('⏭️ Pulando dependentes e área útil (SBPE não possui esses campos)')
      await job.updateProgress(65)
    }
    ...
    if (!clicouAvancar) {
      ...
      if (avancar3) {
        clicouAvancar = true
        console.log('✅ Botão Avançar encontrado (estratégia 3: qualquer elemento clicável)')
      }
    }
    
    if (!clicouAvancar) {
      throw new Error('Botão "Avançar" não encontrado após múltiplas tentativas')
    }
    ...
    const prazoMaximo = await page.evaluate(() => {
      ...
      const prazoLabel = labels.find(l => l.textContent.includes('Prazo Máximo'))
    ...
    await page.waitForSelector('#prazoObra', { state: 'visible', timeout: 5000 })
    ...
    const resultados = await page.evaluate(() => {
      ...
      const tipoFinanciamento = tipoFinanciamentoLabel ? tipoFinanciamentoLabel.textContent.trim() : '';

      // 1. Valor de Avaliação (Valor do imóvel da página)
      const valorAvaliacaoRow = Array.from(document.querySelectorAll('tr')).find(tr => 
        tr.textContent.includes('Valor do imóvel:')
      );
      const valorAvaliacao = valorAvaliacaoRow ? 
        extrairValor(valorAvaliacaoRow.querySelector('td:last-child')?.textContent || '') : '';

      // 2. Subsídio (Desconto)
      const subsidioRow = Array.from(document.querySelectorAll('tr')).find(tr => 
        tr.textContent.includes('Desconto:')
      );
      const subsidio = subsidioRow ? 
        extrairValor(subsidioRow.querySelector('td:last-child')?.textContent || '') : '';

      // 3. Valor Financiado (Valor de Financiamento)
      const valorFinanciadoRow = Array.from(document.querySelectorAll('tr')).find(tr => 
        tr.textContent.includes('Valor de Financiamento')
      );
      let valorFinanciado = '';
      if (valorFinanciadoRow) {
        const tdText = valorFinanciadoRow.querySelector('td:last-child')?.textContent || '';
      ...
      const prazo = prazoRow ? 
        extrairNumero(prazoRow.querySelector('td:last-child')?.textContent || '') : '';
    ...
    console.log('✅ Resultados capturados:', JSON.stringify(resultados, null, 2))
    ...
    const formatarMoeda = (valor) => {
      ...
      const numero = typeof valor === 'number' ? valor : converterValorBrasileiro(valor);
    ...
    let valorFinanciadoNum = converterValorBrasileiro(resultados.valorFinanciado);
    ...
    if (valorFinanciadoNum > valorImovelInput) {
      ...
      console.log(`   Novo valor financiado: R$ ${valorFinanciadoNum.toFixed(2)}`);
    }
    
    console.log('💰 Cálculo da entrada:');
    console.log(`   Valor do Imóvel (input): R$ ${valorImovelInput.toFixed(2)}`);
    console.log(`   Valor de Avaliação (input): R$ ${valorAvaliacaoInput.toFixed(2)}`);
    ...
    resultados.valorAvaliacao = formatarMoeda(converterValorBrasileiro(resultados.valorAvaliacao));
    ...
    resultados.prestacao = formatarMoeda(converterValorBrasileiro(resultados.prestacao));
    // tipoFinanciamento já vem como string, não precisa formatar
    
    console.log('📦 Resultados formatados:', JSON.stringify(resultados, null, 2));
